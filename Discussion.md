...
## 08/02/2026 — Firmware: continuous round-based operation, variable grid size, real instructions ack

### Decision
The rover/base firmware previously handled exactly one grid-down/instructions-back transaction and then stopped (`if(gotInstructions) return;` / `if(sentInstructions) return;`), which never matched the actual target behavior of a rover continuously traversing and re-sending updated maps. Reworked it into a continuous loop of rounds:

- Grid and instructions messageIds are now round-scoped (`GRID_MESSAGE_ID_BASE + roundNumber`, `INSTRUCTIONS_MESSAGE_ID_BASE + roundNumber`) rather than fixed constants, because `PacketReassembler` has no reset -- reusing one fixed ID every round would make `isComplete()` instantly true on round 2 from round 1's leftover data, before any new fragment arrived. `FRAGMENT_STATUS_MESSAGE_ID` stays a fixed type tag; the round it's about travels inside the payload as `subjectMessageId`.
- Rover's packet routing was decoupled from its local ARQ stage. Base doesn't wait for rover to be "ready" -- it sends instructions the moment it has a PC response, which can land while rover is still deep in grid-ARQ retries. The first version gated instructions routing on `gridConfirmed`, so an early-arriving instructions packet was silently dropped and never seen again; this looked like "rover receives round N's instructions in round N+1" during testing. Fixed by routing any incoming packet by message type/id every loop() iteration, independent of which stage flags happen to be set.
- Base guards against round-number regression: a late straggler fragment from an already-finished older round could otherwise snap `lastSeenGridRound` backward and corrupt an in-progress round's decode. Only a round number *greater than* the last one seen triggers state reset; anything lower is ignored outright.
- Jetson->rover grid framing changed from a hardcoded 20x20 assumption (no header) to `[rows:2][cols:2][cells...]`, matching the BASE->computer leg's existing framing. Since base needs the shape too (not just cell count) to forward it correctly, the dimensions ride as a 4-byte prefix on the already-fragmented/ARQ'd values stream rather than becoming a third tracked stream. Both ends reject a header describing 0 cells or more than `MAX_GRID_CELLS`, since a corrupted/desynced header being trusted as a real size was an actual bug hit during testing (a garbled 4-byte read produced a nonsense multi-hundred-thousand-cell grid that blocked forever).
- Instructions leg gained a real ack/retry (rover sends `INSTRUCTIONS_ACK_MESSAGE_ID` naming the round once it's fully reassembled instructions; base waits and resends up to `INSTRUCTIONS_MAX_RETRIES` times on silence) replacing an earlier blind-redundant-retransmission stopgap.
- `firmware/lib/occupancy_grid_lib` (PlatformIO's required `lib/` layout for the compression library) is now regenerated from the canonical `include/`/`src/` by a pre-build script (`firmware/sync_lib.py`, wired via `extra_scripts` in `platformio.ini`) instead of being a manually re-copied second source of truth. The mirror is gitignored; only the canonical copy is tracked.

### Why
All of the above came out of an actual hardware test session, not a design exercise up front -- the round-scoping, the routing decoupling, and the round-regression guard were each fixes for a specific observed failure (respectively: round 2+ silently reusing round 1's reassembler state, instructions arriving before rover was "ready" to accept them, and a late fragment corrupting an in-progress round), not anticipated in advance.

### Timeout tuning (retune expected)
`INSTRUCTIONS_TIMEOUT_MS` (rover's overall per-round give-up budget, counted from grid-send) went from an initial 15s guess to 25s to 60s over the course of testing. The naive sum of the other configured timeouts (grid ARQ retries + instructions ack retries) comes to roughly 25s, but that sum only counts the artificial `delay(300)` pacing between packets, not actual LoRa airtime per transmission -- confirmed on hardware that real rounds were regularly taking close to double the naive budget. 60s is a working number for this bench setup, not a measured/derived one.

### Open issue: intermittent garbage instructions payloads (unresolved)
During testing, some rounds produced implausible waypoint data on the PC side -- reported counts (13, 48, 82 waypoints) far exceeding the fixed 4-waypoint canned test path, with garbage-looking (x,y) values, and the reported count growing round over round within a session. Traced through `packetizer.cpp`/`packet_reassembler.cpp` by hand: fragmentation is deterministic, CRC-16 is checked per fragment before acceptance, `makeKey(messageId, streamId)` has no collision for the message ID ranges in use, and duplicate fragments (e.g. from base's ack-retries) overwrite idempotently rather than accumulate. No logic bug was found by inspection. Added a defensive check on the rover (reject and don't ack an implausible instructions payload -- non-multiple-of-4 length, or over 64 decoded waypoints -- rather than forward it to the PC) so corrupted data can't silently propagate, but this does not address a root cause. Leading hypothesis, not yet isolated: LoRa-level self-interference/cross-talk from the two antennas being tested in close physical proximity, rather than a reassembly bug -- moving the antennas apart was in progress as a diagnostic when the session's hardware access ran out.

### Status
Accepted and implemented for the round-based/variable-size/instructions-ack changes -- confirmed working on hardware for the core round trip. The garbage-instructions-payload issue above is open, unresolved, and should be the first thing checked next time hardware is available (ideally with `ENABLE_SERIAL_DEBUG` and a real Serial2 debug UART wired up, so failures are actually observable instead of inferred from PC-side symptoms).

## 08/03/2026 — Hardware debugging session: missing radio CRC, a broken antenna, and a staged LoRa test harness

### What happened
Picked up the open issue from 08/02/2026. Root-caused two real problems, and neither turned out to be the leading "antenna cross-talk" hypothesis:

1. **`LoRa.enableCrc()` was never called anywhere in this codebase.** Confirmed by grep across the whole firmware tree and by reading `sandeepmistry/LoRa`'s `begin()`/`parsePacket()`, which never touch the SX127x's CRC-request bit on their own. Every layer -- including production `main.cpp` -- accepted radio-level noise misdetected as a "received packet" with zero hardware-level validation. Directly reproduced during testing: a garbled payload arrived at RSSI -118 dBm, right at the sensitivity floor, with nothing to reject it. This is a much better-supported explanation for the original garbage-instructions-payload symptom than antenna cross-talk ever was. Added to every LoRa test sketch and to `main.cpp`'s `initLoRa()`.
2. **One board's antenna solder joint physically broke** during hands-on antenna testing (confirmed unrepairable), and a separately loose 3.3V connection was briefly mistaken for an antenna problem. Isolated by testing one variable at a time with standalone hardware-only sketches (no project protocol at all): both boards' ESP32/SPI/radio-init path was confirmed fully healthy even with the antenna disconnected -- near-field leakage let them "talk" at close range with no antenna, which is a real but non-representative link, not proof the antenna path works.

### Staged test harness (`firmware/src/lora_test/`)
Built smaller-and-safer-first, specifically because a board with no antenna can safely *receive* but must never *transmit* (PA damage risk into an unmatched load):
- `sender_main.cpp`/`receiver_main.cpp`: bare vendor-library send/receive, isolates "can the radios hear each other at all" from this project's own code.
- `grid_sender_main.cpp`/`grid_receiver_main.cpp`, `instructions_sender_main.cpp`/`instructions_receiver_main.cpp`: one-way, single-shot (no ARQ) tests that push a premade grid / canned waypoint list through the *real* rle/splitcodec/packetizer/reassembler pipeline over an actual RF hop, always sending from the antenna-equipped board. Both ends independently print a `Compression:` line (original/compressed bytes, ratio, counts coder) -- matching values on both sides is a content-level corruption check layered on top of CRC, since a silently-wrong decode would very likely produce a different RLE run structure and therefore a different number here.
- Each gets its own PlatformIO environment, isolated from the real `rover`/`base` builds via `build_src_filter` (every test file defines its own `setup()`/`loop()`, which would collide with `main.cpp`'s if compiled together).

### Radio tuning
`LORA_TX_POWER_DBM` (+20, the SX127x's PA_BOOST max) and `LORA_SPREADING_FACTOR` (starts at 7, the library default) added to `pins.h` as shared constants, centralized for the same reason `LORA_FREQUENCY_HZ` already is there -- sender and receiver must agree, and a shared constant is the only way they can't drift apart. Prompted by a link-budget sanity check: at 4 feet, free-space path loss is only ~33 dB, so an observed -80 dBm RSSI (antenna still disconnected at the time) implied a ~64 dB deficit from an ideal link -- confirming that fixing the antenna should be a large effect, not a marginal one.

### Theoretical two-way protocol (`two_way_protocol.h`, `two_way_rover_main.cpp`, `two_way_base_main.cpp`)
Designed over several rounds of discussion and implemented, but **not yet hardware-tested** -- both roles transmit here, so it needs a second working antenna before it can run end to end. Key decisions:
- Round-scoped message IDs (`5000+round` grid/position, `6000+round` responses) for the same reason `main.cpp` already round-scopes its own IDs -- a fixed ID would let a new round appear falsely "complete" from a stale reassembler entry.
- No fragment-level ARQ: base's response is a round-level pass/fail tag (`[count:1][x:2][y:2]*count`, `0xFF` = "bad data, rescan" -- forwarded to Jetson byte-for-byte, no translation). A lost fragment just means the round times out incomplete and gets redone from a fresh Jetson capture rather than resent piecemeal -- a deliberate simplification, since fragment-level loss recovery can't be tested here without two working antennas anyway.
- Rover alternates 500ms send/listen; sending is continue-from-cursor, never restart-from-fragment-1 -- correct given the round-level redo above is the only loss-recovery path, so restart-from-1's extra airtime buys nothing.
- Base defaults to listening; one rolling quiet-period timer (reset by any received fragment, or by resolving a round) unifies "total silence" and "partial data then stalled" into the same evaluate-and-respond behavior, falling back to a fixed wildcard message ID when it never learned a round number to address a response to.
- A hand-traced airtime estimate (not machine-verified -- hit a broken local MSYS2 toolchain trying to compile-and-run the real codec against the test grid) at the firmware's actual SF7/BW125kHz/CR4/5 settings put one full round at roughly 840ms, already over a fixed 500ms window before even raising spreading factor -- confirming multi-cycle fragment spanning over a widened fixed window, since spanning is self-correcting as SF or grid density changes.
- An RSSI floor (-120 dBm, tunable) added as an extra noise gate on top of the CRC fix above, motivated directly by the real spurious noise packet observed during testing rather than a specific theory of its source.
- Position (`x, y` from Jetson) is 2 raw `int16`s, direct little-endian byte packing, no RLE/entropy coding -- same reasoning already applied to waypoints (too little data for run-length structure to exploit).

### Status
CRC fix and antenna/hardware diagnosis: accepted and applied. The garbage-instructions-payload issue from 08/02/2026 is very likely resolved by the CRC fix, but not confirmed on hardware yet -- needs a working antenna again to retest the original round-trip scenario. Two-way protocol: implemented and compiling, explicitly not yet hardware-tested. Next real milestone: get a second working antenna (the broken solder joint is confirmed unrepairable, needs a replacement module) and run `lora_test_two_way_rover`/`lora_test_two_way_base` for the first time.
